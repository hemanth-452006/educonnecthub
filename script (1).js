...
/* =============================================
   FORCE STUDENT MESSAGE SENDER NAME = "Aman"
   ============================================= */

// Force the sender name globally
localStorage.setItem("currentUserName", "Aman");

// Whenever the message modal opens, set sender = Aman
document.addEventListener("click", (e) => {
  if (e.target.closest(".msg-btn") || e.target.classList.contains("msg-floating")) {
    const senderField = document.getElementById("msg-sender");
    if (senderField) senderField.value = "Aman";
  }
});

// When submitting the form, force sender = Aman
...
// ---------------- Sectioned Announcements (A / B / C) ----------------
(function () {
  const input = document.getElementById('announceInput');
  const postButtons = Array.from(document.querySelectorAll('.post-section-btn'));
  const viewSelect = document.getElementById('viewSection');
  const listEl = document.getElementById('announcementList');

  if (!input || !postButtons.length || !viewSelect || !listEl) {
    console.warn('Announcements elements missing; check HTML IDs/classes.');
    return;
  }

  // helper: read/write section announcements from localStorage
  function getAnnouncementsFor(section) {
    try {
      return JSON.parse(localStorage.getItem('announcements_section_' + section)) || [];
    } catch (e) {
      return [];
    }
  }
  function setAnnouncementsFor(section, arr) {
    try { localStorage.setItem('announcements_section_' + section, JSON.stringify(arr)); }
    catch (e) { console.warn('Could not save announcements', e); }
  }

  // render announcements for the currently-selected view section
  function renderAnnouncements() {
    const sec = viewSelect.value || 'A';
    const items = getAnnouncementsFor(sec);
    if (!items.length) {
      listEl.innerHTML = `<div style="padding:10px; border-radius:6px; background:#f1f5f9; color:#475569;">No announcements for Section ${sec}.</div>`;
      return;
    }
    listEl.innerHTML = items.map(a => {
      const time = new Date(a.t).toLocaleString();
      // each announcement rendered with subject (if provided) and body
      return `
        <div style="background:#e6f0ff; padding:12px; border-radius:8px; box-shadow: 0 2px 6px rgba(0,0,0,0.03);">
          <div style="font-size:12px; color:#475569; margin-bottom:6px;">${a.postedBy ? escapeHtml(a.postedBy) + ' · ' : ''}${time}</div>
          <div style="font-size:15px; color:#0f172a; white-space:pre-wrap;">${escapeHtml(a.text)}</div>
        </div>
      `;
    }).join('');
  }
  ...
  // post handler (section-specific)
  postButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      const sec = btn.dataset.section || 'A';
      const txt = input.value.trim();
      if (!txt) { alert('Please enter an announcement before posting.'); input.focus(); return; }

      // optional: postedBy from current teacher display name (if you store it)
      const postedBy = localStorage.getItem('currentTeacherName') || document.getElementById('teacherName')?.textContent?.trim() || 'Teacher';

      const arr = getAnnouncementsFor(sec);
      arr.unshift({ text: txt, t: new Date().toISOString(), postedBy });
      setAnnouncementsFor(sec, arr);

      // clear input and re-render (if viewing same section)
      input.value = '';
      if (viewSelect.value === sec) renderAnnouncements();
      // small toast replacement
      const msg = `Announcement posted for Section ${sec}.`;
      // use alert for now (or replace with toast)
      alert(msg);
    });
  });

  // when the view section changes, re-render
  viewSelect.addEventListener('change', renderAnnouncements);

  // initial render for default selected section
  renderAnnouncements();
